"""Command dispatcher for subtitle-toolkit."""

from __future__ import annotations

import argparse
import sys

from .caption import main as caption_main
from .removal import main as removal_main
from .residual import main as residual_main


def main(argv: list[str] | None = None) -> int:
    arguments = list(sys.argv[1:] if argv is None else argv)
    parser = argparse.ArgumentParser(prog="subtitle-toolkit")
    parser.add_argument("command", choices=("caption", "remove", "splice"))
    if not arguments or arguments == ["-h"] or arguments == ["--help"]:
        parser.parse_args(arguments)
        return 0
    command, remainder = arguments[0], arguments[1:]
    if command not in ("caption", "remove", "splice"):
        parser.error(
            f"argument command: invalid choice: {command!r} "
            "(choose from 'caption', 'remove', 'splice')"
        )
    handlers = {"caption": caption_main, "remove": removal_main, "splice": residual_main}
    return handlers[command](remainder)


if __name__ == "__main__":
    raise SystemExit(main())
